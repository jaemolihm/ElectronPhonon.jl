...
#   eigvals_of : result -> (nw, nk) eigenvalue array (identity, or `first` for (W, V))
# ---------------------------------------------------------------------------
function bench(name, driver, eigvals_of)
    nrep = 5
    Wc = eigvals_of(driver(ham_cpu))                                       # warmup + reference
    t_cpu = minimum(@elapsed(driver(ham_cpu)) for _ in 1:nrep)

    driver(ham_gpu)                                                        # warmup
    Wg = eigvals_of(driver(ham_gpu))
    t_gpu = minimum(CUDA.@elapsed(CUDA.@sync driver(ham_gpu)) for _ in 1:nrep)

    d = maximum(abs.(sort(Array(Wg), dims=1) .- sort(Wc, dims=1)))
...
bench("valueonly", ham -> get_el_eigen_valueonly_batched(ham, kpts), W -> W);
bench("eigen",     ham -> get_el_eigen_batched(ham, kpts),           WV -> first(WV));
