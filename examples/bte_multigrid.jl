...
#   1. build the k `FilteredBandStates` with `filter_electron_states_multigrid` (fine + coarse spec); it carries
...
#   2. build the k+q `FilteredBandStates` as the explicit symmetry unfold of the k selection
...
# Auto-μ: nlist set, μlist left unset so the driver solves μ. The multigrid `FilteredBandStates` carries
...
`nk_wide`. Builds the k `FilteredBandStates` (double grid) and the full-BZ k+q selection (its symmetry
unfold).
"""
